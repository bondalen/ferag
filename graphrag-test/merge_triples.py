...
def main() -> None:
    ...
    merged = Graph()
    # Копируем все префиксы из g1
    for p, n in g1.namespace_manager.namespaces():
        merged.bind(p or "ferag", n)

    # 1) Добавляем все триплеты из цикла 1
    for t in g1:
        merged.add(t)

    # 2) Коллизия описаний: (s, ferag:description, o) — при совпадении s оставляем значение из цикла 2
    from rdflib import URIRef
    desc_uri = URIRef(FERAG + "description")
    from_uri = URIRef(FERAG + "from")
    to_uri = URIRef(FERAG + "to")
    weight_uri = URIRef(FERAG + "weight")
    rel_type = URIRef(FERAG + "Relationship")

    descriptions_g2 = list(g2.triples((None, desc_uri, None)))
    for s, p, o in descriptions_g2:
        if not isinstance(s, BNode):
            # Удалить из merged все (s, description, *) и добавить (s, description, o) из g2
            to_remove = list(merged.triples((s, desc_uri, None)))
            for t in to_remove:
                merged.remove(t)
            merged.add((s, p, o))

    # 3) Остальные триплеты из g2: добавить, если такого (s,p,o) ещё нет (дедупликация)
    added = 0
    skipped_dup = 0
    for s, p, o in g2:
        if (s, p, o) in merged:
            skipped_dup += 1
            continue
        # Не дублируем описание — уже обработано
        if p == desc_uri and not isinstance(s, BNode):
            continue
        merged.add((s, p, o))
        added += 1

    # 4) Дедупликация связей (Relationship): одинаковые (from, to, description) — оставить один blank node
    # Собираем по (from_uri, to_uri, desc_literal) все BNode
    ...
    # Оставляем один BNode на ключ, остальные удаляем (вместе с их триплетами)
