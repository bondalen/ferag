...
**Приоритет:** сначала достичь диалога с LLM по графу (6.21/6.22 — минимальный контур: Fuseki + LLM), затем при необходимости — проекции (6.18–6.20).
...
| 6.21 | Реализовать RAG для диалога (Hybrid Retriever) | [ ] | **Следующий рубеж.** Минимум: запрос к ferag-prod (SPARQL) + контекст → LLM → ответ. План: docs/chats/26-0213-1049_plan.md |
| 6.22 | Протестировать диалог: задать вопрос → получить ответ | [ ] | Проверка: знания из графа → RAG → ответ пользователю |
