...
| 9.13 | (опц.) Сжатие длинных диалогов | [ ] | Периодическая суммаризация старых сообщений, подстановка резюме + последние K пар в запрос к LLM. Покрывает и длинные диалоги, и контекст последних реплик (цель 9.12). Зависимость: 9.11. [PROJECT-004-chat-dialogue](../project/PROJECT-004-chat-dialogue.md). Завершает сегмент подготовки ответов (после улучшения retrieval и контекста диалога). |
| 9.14 | Отмена задачи загрузки пользователем | [ ] | Сохранение celery_task_id при старте цепочки; эндпоинт POST /tasks/{id}/cancel; кнопка «Отменить загрузку» в UI; публикация в Redis при отмене. [PROJECT-004](../project/PROJECT-004.md) разд. 9 «Загрузка и индексация: доработки». Обсуждение: план 26-0227-1338. |
| 9.15 | Лимиты времени задач индексации и оценка при старте | [ ] | Лимиты 3–5 ч (конфигурируемо через env); расчёт оценки времени при создании задачи; предупреждение в UI при риске превышения лимита (~90%). [PROJECT-004](../project/PROJECT-004.md) разд. 9. |
| 9.16 | Прогресс загрузки деревом этапов в UI | [ ] | Отображение четырёх этапов (GraphRAG → Schema Induction → Merge → Staging) деревом/чек-листом с отметками по WebSocket. [PROJECT-004](../project/PROJECT-004.md) разд. 9. |
| 9.17 | Накопление статистики загрузок для прогноза времени | [ ] | Таблица замеров (task_id, rag_id, размер входа, длительность, статус); запись при завершении задачи; калибровка оценки времени. [PROJECT-004](../project/PROJECT-004.md) разд. 9. |
