...
- **Для архитектуры:** иметь законченный сегмент подготовки ответов (retrieval + диалог + сжатие), к которому долго не нужно возвращаться при доработках других подсистем (визуализация, проекции и т.п.).
...
3. **Улучшение retrieval перед диалогом**  
   Параллельно с доработкой диалога требуется улучшить индексацию и retrieval по графу: добиться большей полноты сущностей и фактов в Fuseki (в т.ч. по тестовому датасету ferag-00026) и более осмысленного выбора контекста (`rag_context.py`). Для этого используется утилита `diagnose_rag_context.py`, отдельные задачи в Блоке 6/9 TASKS.md. Без этого блока даже идеальный диалог и сжатие не дадут корректных ответов.

4. **По желанию позже:** лимит по токенам/символам; сессии диалога (session_id); выборочная подстановка по релевантности (см. разд. 5–7).
...
- **Приоритеты для сегмента ответов:** (1) улучшить индексацию и retrieval (полнота графа и качество контекста), (2) улучшить многотуровый диалог (подмешивание последних реплик поверх RAG-контекста), (3) реализовать сжатие длинных диалогов (9.13). После выполнения этих трёх шагов сегмент подготовки ответов считается стабильным.

Задачи в [TASKS.md](../tasks/TASKS.md) (Блок 6/9: улучшение retrieval и RAG; Блок 9: 9.11, 9.12, 9.13).
