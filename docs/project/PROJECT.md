...
- **Загрузка и индексация (запланированные доработки):** отображение прогресса этапами (дерево GraphRAG → Schema Induction → Merge → Staging), лимиты времени задач 3–5 ч и оценка при старте загрузки, предупреждение в UI при риске таймаута, отмена задачи пользователем, накопление статистики для калибровки прогноза времени. Подробнее: [PROJECT-004](PROJECT-004.md), раздел «Загрузка и индексация: доработки»; при реализации — задачи в [TASKS.md](../tasks/TASKS.md) (Блок 9).
...
- **Быстрый деплой:** порядок действий AI-агента при выкатке на cr-ubu — [PROJECT-004, п. 12](PROJECT-004.md), [deploy/DEPLOYMENT_SUMMARY.md](../../deploy/DEPLOYMENT_SUMMARY.md), правило [.cursor/rules/deploy-cr-ubu-ssh.mdc](../../.cursor/rules/deploy-cr-ubu-ssh.mdc).
