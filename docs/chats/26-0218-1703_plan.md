...
**Исполнение:** в `app/models.py` добавлены модели `User`, `RagInstance`, `RagMember`, `UploadCycle`, `Task` с полями по схеме плана; `__tablename__` в snake_case; внешние ключи через `ForeignKey`; индексы на `email` (users) и `owner_id` (rag_instances). Проверка: импорт моделей регистрирует все 5 таблиц в `Base.metadata`.
...
**Исполнение:** выполнены `alembic init alembic`; в `alembic/env.py` заданы `target_metadata = Base.metadata`, импорт `app.models`, подстановка `sqlalchemy.url` из `get_settings().database_url`; создана миграция `8355f4b3f5a9_initial.py`, применена `alembic upgrade head`. Проверка: в БД видны таблицы `users`, `rag_instances`, `rag_members`, `upload_cycles`, `tasks` и `alembic_version`; повторный `alembic upgrade head` выполняется без ошибок.
...
**Исполнение:** в `app/auth.py` добавлены `hash_password` / `verify_password` (passlib CryptContext bcrypt), `create_access_token` (jwt.encode, exp из `jwt_expire_minutes`), `decode_access_token` (jwt.decode, при истечении/невалидности — JWTError). В `requirements.txt` зафиксировано `bcrypt<5` из-за несовместимости passlib с bcrypt 5.x. Проверка: хеш/верификация пароля и создание/декодирование токена проходят.
...
**Исполнение:** в `app/deps.py` добавлены реэкспорт `get_db` из `app.db`, `OAuth2PasswordBearer(tokenUrl="auth/login")`, зависимость `get_current_user`: декодирование токена, извлечение `sub` → `user_id`, `db.get(User, user_id)`; при JWTError/ValueError или отсутствии пользователя — HTTP 401. Проверка: импорт модуля проходит.
...
**Исполнение:** добавлены `app/routers/__init__.py`, `app/routers/auth.py`: Pydantic-схемы (RegisterBody, LoginBody, UserResponse, TokenResponse), эндпоинты `POST /auth/register` (409 при занятом email), `POST /auth/login` (JSON body), `GET /auth/me` (get_current_user); роутер подключён в `main.py` с prefix `/auth`. В `requirements.txt` — `email-validator` для EmailStr. Проверка: register → login → me через TestClient и дубликат email → 409.
...
**Исполнение:** в `app/fuseki_admin.py` реализованы функции с `httpx.Client` и basic auth из `get_settings()`: `create_dataset` (POST form, 409 не ошибка), `delete_dataset` (DELETE), `list_datasets()` (GET, имена без ведущего слэша). Проверка: create → list содержит датасет → delete → список без него; повторный create идемпотентен.
...
**Исполнение:** добавлены `app/routers/rags.py` (POST/GET/GET by id/DELETE), проверка доступа (owner или member), при создании — flush → `rag_prod_dataset(rag.id)` → commit → `create_dataset`; при удалении — проверка отсутствия задач со status running, удаление записи, `delete_dataset` в try/except. Роутер подключён в `main.py`. Проверка: create → `fuseki_dataset = "ferag-00001"`, датасет в Fuseki; list/get/delete — OK.
...
**Исполнение:** в `app/fuseki_admin.py` добавлены `rag_prod_dataset(rag_id)`, `rag_staging_dataset(rag_id, cycle_n)`, `rag_triples_dataset(rag_id, cycle_n)`, `rag_ontology_dataset(rag_id, cycle_n)` по формулам плана.
...
**Исполнение:** добавлены `app/routers/tasks.py`: `GET /tasks/{task_id}` (проверка доступа к RAG задачи через `_can_access_rag`), `GET /rags/{rag_id}/tasks` (skip/limit, проверка доступа к RAG); модель ответа `TaskResponse`; роутер подключён в `main.py` без prefix. Проверка: список задач по RAG (пустой и с одной задачей), получение задачи по id, 404 для несуществующей задачи.
...
**Исполнение:** проверка выполнена автоматически (TestClient + Fuseki list_datasets): register → login → POST /rags → GET /rags → наличие датасета в Fuseki → DELETE /rags/{id} → 204 → датасет в Fuseki исчез → GET /rags пустой. Ваших действий не потребовалось. Повторить сценарий вручную через Swagger: запустить `./venv/bin/uvicorn app.main:app --reload --port 47821` из `code/backend`, открыть `http://localhost:47821/docs` и выполнить шаги 1–7.
