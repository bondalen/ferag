...
- Папки `backend/`, `worker/`, `frontend/` не существуют.

**Окружение для разработки (этот чат):** локально в WSL2 на nb-win, `uvicorn` из виртуального окружения `backend/venv`. PostgreSQL и Fuseki — те же локальные экземпляры. Docker, cr-ubu — для Чата 3.

---

## Задача плана

Реализовать **backend skeleton**: структура `backend/`, зависимости, схема БД с Alembic, аутентификация (register/login/JWT), CRUD RAG-экземпляров (создание датасета в Fuseki при создании RAG), эндпоинт polling-статуса задач. По завершении: `uvicorn` запущен, Swagger UI доступен, сценарий «register → login → create RAG → list RAGs» работает, датасет в Fuseki создаётся.
...
#### 1.1 Создать `backend/` и виртуальное окружение

Создать каталог `backend/`, виртуальное окружение, установить зависимости из `requirements.txt`. Создать `.env` из `.env.example` и заполнить реальными значениями для локальной разработки.
...
`app = FastAPI(title="ferag API", root_path="/ferag/api")`. Пока без роутеров — только `GET /health` → `{"status": "ok"}`. Запуск: `uvicorn app.main:app --reload --port 47821`.
...
**Задача:** Реализовать `frontend/` (Vue 3 + Vite). Собрать и задеплоить на cr-ubu. Запустить всё через Docker.

**Ключевые шаги:**
- `frontend/` структура, Vite `base: '/ferag/'`, роутер, pinia, axios.
...
- `backend/Dockerfile` — python:3.11-slim + redis-server + supervisor (из `supervisord.conf`).
